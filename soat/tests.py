...
class SoatPublicTests(TestCase):
    def setUp(self):
        ...
        self.reference = Path(self.temp.name) / "SOAT_prueba_3_Def.xlsx"
        self.reference.write_bytes(workbook_bytes([
            HEADERS,
            ["ABC123", "Angelina", "", "", "A&S", "", "", "2030-01-01", "Activo", "Vigente", "Por venta", "MOV-1", "", "", "", "", "Movilidad", "", "", "", "", "", "", "200"],
        ], "Movilidad"))
    ...
    @override_settings(SOAT_REFERENCE_FILE="")
    def test_missing_reference_is_functional_error(self):
        response = self.client.post(reverse("soat:upload"), {"source_file": self.upload()})
        self.assertEqual(response.status_code, 422)
        self.assertContains(response, "archivo de referencia requerido", status_code=422)

    def test_invalid_name_and_invalid_excel_are_rejected(self):
        with override_settings(SOAT_REFERENCE_FILE=self.reference):
            wrong = self.client.post(reverse("soat:upload"), {"source_file": self.upload("otro.xlsx")})
            corrupt = self.client.post(reverse("soat:upload"), {"source_file": self.upload(content=b"not-xlsx")})
        self.assertEqual(wrong.status_code, 422)
        self.assertContains(wrong, "conservar el nombre", status_code=422)
    ...
    def test_concurrent_processing_is_isolated(self):
        def run():
            return process_soat(self.upload(), reference_path=self.reference).content
    ...
    def test_seven_management_criteria_match_delivered_script(self):
        cases = [
