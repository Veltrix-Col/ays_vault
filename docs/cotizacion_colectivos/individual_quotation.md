...
una empresa o persona, abre una póliza, selecciona el afiliado confirmado y
...
```text
Cliente → póliza → afiliado confirmado → enlace firmado
...
3. Los afiliados disponibles se deduplican con referencias HMAC del Snapshot;
   nunca por nombre o documento.
...
7. El servidor valida de nuevo que la póliza, el ramo y el afiliado pertenezcan
   al mismo Workspace antes de persistir.
