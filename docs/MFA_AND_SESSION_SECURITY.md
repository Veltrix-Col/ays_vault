...
Los propósitos administrativos actuales incluyen `cards_manage`, `identity_admin`, `session_manage`, `device_manage`, `alerts_manage`, `password_change`, `mfa_manage`, `policy_admin` y `outside_hours`. El MFA se valida una vez durante el ingreso y abre inmediatamente una única `SensitiveOperationWindow` fija de 30 minutos, ligada al usuario y al hash de la sesión. Al expirar, la siguiente operación sensible solicita solo la contraseña dentro de la misma sesión; no ejecuta `login()`, no rota la clave de sesión y no repite OTP. Los propósitos delimitan la intención y la auditoría, pero una ventana vigente es transversal a las operaciones autorizadas. La vigencia nunca se extiende con la actividad.
...
Empresa, PAN y vencimiento mantienen autorizaciones adicionales. Cada operación de revelado/copia exige el campo único `Número certificado recibo - Zoho` y crea un `ProtectedOperationContext` temporal exclusivo para esa tarjeta. Para conservar compatibilidad histórica, el modelo mantiene `reason` con una descripción fija segura y guarda el número Zoho en `internal_reference`; la interfaz nunca vuelve a pedir dos valores. Los tres campos comparten ese contexto mientras se permanezca en la misma operación. Volver a la Bóveda, confirmar otra tarjeta, expirar la ventana o invalidar la sesión cierra el contexto. `RevealGrant` limita cada campo; el valor se retira del DOM a los 20 segundos o al ocultarse/cambiar de pestaña y la copia mantiene token de un solo uso.
