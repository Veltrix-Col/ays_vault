...
class EmailExceptionsAccessTests(TestCase):
    ...
    def test_local_public_access_uses_the_inherited_gate_without_module_permission(self):
        response = self.client.get(reverse("email_exceptions:list"))
        self.assertEqual(response.status_code, 200)
    ...
    def test_valid_sso_without_module_permission_can_use_module(self):
    ...
    def test_valid_sso_can_operate_without_module_permissions(self):
        self.sso_user.user_permissions.clear()
        with patch("email_exceptions.views.responsible_options") as zoho_read:
            ...
            self.assertEqual(self.client.post(reverse("email_exceptions:ignore", args=[self.exception.pk]), {"reason": "Otro"}).status_code, 302)
            self.assertEqual(self.client.post(reverse("email_exceptions:create_task", args=[self.exception.pk]), {}).status_code, 302)
    ...
    def test_sso_identity_never_needs_vault_profile_or_module_permissions(self):
        self.sso_user.refresh_from_db()
        self.assertFalse(self.sso_user.has_perm(VIEW_PERMISSION))
        self.assertFalse(self.sso_user.has_perm("email_exceptions.operate_email_exceptions"))
    ...
    @override_settings(DEBUG=True, RUNNING_TESTS=True, TOOLS_ACCESS_MODE="local_public")
    def test_local_inherited_access_covers_direct_exception_and_case_urls(self):
        self.user.user_permissions.clear()
        self.client.force_login(self.user)
        self.assertEqual(self.client.get(reverse("email_exceptions:detail", args=[self.exception.pk])).status_code, 200)
        self.assertEqual(self.client.get(reverse("email_exceptions:case_detail", args=[1])).status_code, 404)

    @override_settings(DEBUG=True, RUNNING_TESTS=True, TOOLS_ACCESS_MODE="local_public")
    def test_local_inherited_access_does_not_depend_on_vault_role(self):
        self.user.user_permissions.clear()
