...
class ProductToolsContractTests(SimpleTestCase):
    ...
    def test_novelty_review_and_processing_errors_are_presented_separately(self):
        ...
        self.assertIn("novelty_ingress_retry_create_person", content)
        self.assertIn("Reintentar creación", content)
        self.assertIn("ingress.reconcile_required", content)
        self.assertIn("review_status == 'NOT_FOUND'", content)
